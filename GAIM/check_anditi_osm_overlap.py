...
df_inference_tiles = pd.read_csv("inference_overlap_unfiltered.csv")
df_inference_preds = pd.read_csv("inference_vietnam.csv")
...
for i1 in tqdm(range(len(df_anditi))):
    ...
    filtered_inference = df_inference_tiles[distance < 200].copy()
...
df_inference_preds_values.to_csv("oai_inference_max_pred.csv")
...
df_inference_distance_values.to_csv("oai_inference_min_distance.csv")
