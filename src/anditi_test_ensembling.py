...
def inference(c, exp, finetune):
    ...
    out_file_school = os.path.join(exp_dir, "anditi_school_ensembling_tencrop500_max.csv")
    out_file_non_school = os.path.join(exp_dir, "anditi_non_school_ensembling_tencrop500_max.csv")
    ...
    for i in tqdm(range(len(images_school))):
        ...
        row = {"image": f"{i}.jpeg","pred": positive_prob, "lon":lon, "lat":lat}
        df_school.loc[i] = row
    ...
    for i in tqdm(range(len(images_non_school))):
        ...
        row = {"image": f"{data.iloc[i]['UID']}.jpeg","pred": positive_prob, "lon":lon, "lat":lat}
    ...
    log_file = os.path.join(exp_dir, "anditi_results_ensembling_tencrop500_max.txt")
    f = open(log_file, "w")
    f.write(log_string)
    f.close()
...
def main():
    ...
    parser.add_argument("-f", "--fine_tune", default="")
