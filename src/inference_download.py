...
def download_sat_images(
    ...
    for district in data:
        if not os.path.exists(f"{dest_dir}/{district}"):
            os.makedirs(f"{dest_dir}/{district}")
        for index in range(len(data[district])):
            image_file = f"{dest_dir}/{district}/{index}.jpeg"
            d1 = data[district][index]["max_lon"]-data[district][index]["min_lon"]
            d2 = d1 * math.sqrt(2)
            d = d2-d1
            bbox = (
                        data[district][index]["min_lon"] - d/2,
                        data[district][index]["min_lat"] - d/2,
                        data[district][index]["max_lon"] + d/2,
                        data[district][index]["max_lat"] + d/2,
                    ) 
