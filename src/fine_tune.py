...
def main(c, exp_name="all",  fine_tune_dataset_name=None):    
    ...
    # Load dataset
    phases = ["train", "test"]
    data, data_loader, classes = cnn_utils.load_dataset(config=c, phases=phases, name = fine_tune_dataset_name)
    ...
    exp_dir = os.path.join(exp_dir, f"fine_tune_{fine_tune_dataset_name}")
    if not os.path.exists(exp_dir):
        os.makedirs(exp_dir)
...
def test(config, exp_name="all", dataset_name=None, fine_tune_dataset_name=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-e', "--exp_name", default="global_no_vietnam_500images_no_lowres_continuous_rotation_0-90_crop352_no_AMP_convnext_small")
    ...
    test_flag = args.test
    if test_flag:
        test(c, args.exp_name, args.fine_tune_dataset)
    else:
