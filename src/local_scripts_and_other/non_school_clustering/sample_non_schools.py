...
if __name__ == "__main__":
    ...
    # Normalize the distances from centroid 1
    min_distance1 = dataset_ns["c1_dist"].min()
    max_distance1 = dataset_ns["c1_dist"].max()
    dataset_ns["c1_norm_dist"] = (dataset_ns["c1_dist"] - min_distance1) / (max_distance1 - min_distance1)

    # Calculate probability to belong to centroid 1 (1 / d1)
    dataset_ns["c1_prob"] = 1 / (dataset_ns["c1_norm_dist"] + 1e-10)
    ...
    # Normalize the distances from centroid 2
    min_distance2 = dataset_ns["c2_dist"].min()
    max_distance2 = dataset_ns["c2_dist"].max()
    dataset_ns["c2_norm_dist"] = (dataset_ns["c2_dist"] - min_distance2) / (max_distance2 - min_distance2)

    # Calculate probability to belong to centroid 2 (1 / d2)
    dataset_ns["c2_prob"] = 1 / (dataset_ns["c2_norm_dist"] + 1e-10)
    ...
    cluster2_ns.drop(columns=["c1_dist", "c1_norm_dist", "c1_prob"], inplace=True)
